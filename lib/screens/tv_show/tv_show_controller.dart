...
class TvShowController extends GetxController {
  ...
  Rx<Future<TvShowModel>> getTvShowDetailsFuture = Future(
    () => TvShowModel(data: TvShowDetailsModel(yourReview: ReviewModel())),
  ).obs;
  Rx<Future<RxList<EpisodeModel>>> getEpisodeListFuture = Future(
    () => RxList<EpisodeModel>(),
  ).obs;
  Rx<Future<EpisodeModel>> getEpisodeFuture = Future(
    () => EpisodeModel(plan: SubscriptionPlanModel()),
  ).obs;

  Rx<TvShowDetailsModel> tvShowDetail = TvShowDetailsModel(
    yourReview: ReviewModel(),
  ).obs;
  Rx<VideoPlayerModel> showData = VideoPlayerModel().obs;
  Rx<SeasonModel> selectSeason = SeasonModel().obs;

  Rx<EpisodeModel> selectedEpisode = EpisodeModel(
  ...
  RxInt currentEpisodeIndex = 0.obs;
  RxInt page = 1.obs;
  ...
  Future<void> onInit() async {
    if (Get.arguments is VideoPlayerModel) {
      ...
      if ((Get.arguments as VideoPlayerModel).type == VideoType.tvshow &&
          (Get.arguments as VideoPlayerModel).episodeId == -1) {
        isTrailer(true);

        await getTvShowDetail(
          showLoader: false,
          tvShowId: (Get.arguments as VideoPlayerModel).entertainmentId != -1
              ? (Get.arguments as VideoPlayerModel).entertainmentId
              : (Get.arguments as VideoPlayerModel).id,
        );
      } else {
        isTrailer(true);
        selectedEpisode(
          EpisodeModel.fromJson((Get.arguments as VideoPlayerModel).toJson()),
        );
    ...
    await getTvShowDetailsFuture(
        ...
        .then((value) async {
          ...
          if (value.data.tvShowLinks.isNotEmpty) {
            selectSeason(
              isPlayingEpisode.isTrue
                  ? tvShowDetail.value.tvShowLinks.firstWhere(
                      (element) =>
                          element.seasonId == selectedEpisode.value.seasonId,
                    )
                  : tvShowDetail.value.tvShowLinks.first,
            );
    ...
    await getEpisodeListFuture(
        ...
        .then((value) {
          if (selectedEpisode.value.id > 0) {
            currentEpisodeIndex(
              episodeList.indexWhere(
                (element) => element.id == selectedEpisode.value.id,
              ),
            );
          }
  ...
  void editReview() {
    ...
    movieDetCont.ratingVal(
      double.parse(tvShowDetail.value.yourReview.rating.toString()),
    );
  ...
  Future<void> deleteReview() async {
    ...
    CoreServiceApis.deleteRating(
          request: {"id": tvShowDetail.value.yourReview.id},
        )
        .then((value) async {
          await getTvShowDetail();
          final AddReviewController reviewControllerCont =
  ...
  Future<void> saveWatchList({bool addToWatchList = true}) async {
    ...
    if (addToWatchList) {
      ...
      tvShowDetail.value.isWatchList = false;
      CoreServiceApis.deleteFromWatchlist(idList: [tvShowDetail.value.id])
          .then((value) async {
            await getTvShowDetail();
  ...
  Future<void> checkIfAlreadyDownloaded() async {
    showDownload(
      selectedEpisode.value.downloadStatus.getBoolInt() &&
          selectedEpisode.value.downloadQuality.isNotEmpty,
    );
    ...
    if (selectedEpisode.value.downloadQuality.length == 1 &&
        selectedEpisode.value.downloadQuality.first.quality ==
            QualityConstants.defaultQuality &&
        (selectedEpisode.value.downloadType != URLType.url &&
            selectedEpisode.value.downloadType != URLType.local)) {
      showDownload(false);
    }

    bool downloaded = await checkIfDownloaded(
  ...
  void playNextEpisode(EpisodeModel episode) {
    isTrailer(false); // Ensure we're not in trailer mode
    ...
    LiveStream().emit(changeVideoInPodPlayer, [
      episode.videoUrlInput,
      false,
      episode.videoUploadType,
      null,
      getVideoPlayerResp(episode.toJson()),
    ]);
  }

  void playPreviousEpisode() {
    int currentIndex = episodeList.indexWhere(
      (e) => e.id == selectedEpisode.value.id,
    );
    if (currentIndex > 0) {
